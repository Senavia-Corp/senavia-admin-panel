...
import type { BillingRecord, BillingStatus } from "@/types/billing-management"

interface BillingTableProps {
  billingRecords: BillingRecord[]
  onAddBilling: () => void
  onViewBilling: (billing: BillingRecord) => void
  onDeleteBilling: (billing: BillingRecord) => void
...
export function BillingTable({
  ...
  const statuses: BillingStatus[] = ["Created", "Processing", "InReview", "Rejected", "Accepted", "Invoice", "Paid"]
    ...
    <div className="flex flex-col h-full space-y-6 w-full">
      ...
      <Card className="bg-gray-900 text-white flex-shrink-0 w-full">
        <CardHeader className="flex flex-row items-center justify-between py-6 px-8">
          <div>
            <h2 className="text-xl font-semibold">Add Billing Record</h2>
      ...
      <Card className="bg-gray-900 text-white flex-1 flex flex-col min-h-0 w-full">
        <CardHeader className="flex-shrink-0 px-8">
          <div className="flex items-center justify-between w-full">
            <div>
              <h2 className="text-xl font-semibold">All Billing Records</h2>
              <p className="text-gray-400">Description</p>
            </div>
