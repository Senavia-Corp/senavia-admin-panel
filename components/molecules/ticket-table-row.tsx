...
export function SupportTableRow({ ticket, onView, onDelete }: SupportTableRowProps) {
  return (
    <tr className="border-b border-gray-200 hover:bg-gray-50">
      ...
      <td className="w-48 px-6 py-4 text-sm text-gray-900 truncate">{ticket.title}</td>
      ...
      <td className="w-32 px-6 py-4">
        <div className="flex space-x-2">
          <ActionButton type="view" onClick={() => onView(ticket)} className="text-gray-700 hover:text-gray-900" />
          <ActionButton type="delete" onClick={() => onDelete(ticket)} className="text-gray-700 hover:text-gray-900" />
        </div>
      </td>
    </tr>
  )
